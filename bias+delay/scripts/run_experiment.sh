...
Options:
  --mode baseline|bias|delay|fullchain
...
if [[ "${MODE}" != "baseline" && "${MODE}" != "bias" && "${MODE}" != "delay" && "${MODE}" != "fullchain" ]]; then
  echo "[run_experiment] --mode must be baseline, bias, delay, or fullchain" >&2
  exit 1
fi
...
need_rebuild=0
required_bins=(controller sensor attacker_bias attacker_delay)
