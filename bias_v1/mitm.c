...
static const char* GAIN_PATH = "/tmp/cps_bias_v1_gains.json";
...
static void maybe_tamper_gains(int enable) {
    ...
    write_atomic("/tmp/cps_bias_v1_attack_marker.txt", "MITM tampering gains\n");
    ...
    pid_t pid = fork();
    if (pid == 0) {
        execl("/bin/sh", "sh", "-c", "echo helper_executed >> /tmp/cps_bias_v1_attack_marker.txt", (char*)NULL);
        _exit(0);
    }
}
