...
if [[ "${MODE}" != "baseline" && "${MODE}" != "bias" && "${MODE}" != "fullchain" ]]; then
  echo "[run_experiment] --mode must be baseline, bias, or fullchain" >&2
  exit 1
fi
...
need_rebuild=0
required_bins=(controller sensor attacker_bias)
...
if [[ "${MODE}" != "baseline" && "${PTRACE_COMPAT}" == "auto" ]]; then
